...
def write_fifo(event,owner):
    with open(g_fifo, "w") as file:
        print("Writing event to fifo")
        file.write(time.time(),event,owner)
        file.close()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if mac not in g_state:
            ...
            g_state[mac] = {"detected":time.time(), "status":0, "rssi": [rssi]}
        else:
            g_state[mac]["rssi"] += [rssi]
            if len(g_state[mac]["rssi"]) > 3:
                g_state[mac]["rssi"].pop(0)
        g_state[mac]["avg_rssi"] = mean(g_state[mac]["rssi"])
