...
https://raw.githubusercontent.com/dbader/schedule/master/test_schedule.py
"""
import datetime
import functools
import mock
...
import tz_schedule
from tz_schedule import every
from schedule import ScheduleError, ScheduleValueError, IntervalError, CancelJob
...
class TestTimezoneAwareScheduler(unittest.TestCase):
    def setUp(self):
        tz_schedule.clear()

    def test_summer(self):
        tz_schedule.every("Monday").at("09:00 BST").do(nothing)
        print(tz_schedule.next_run())
    ...
    def test_time_units(self):
        ...
        job_instance = tz_schedule.Job(interval=2)
    ...
    def test_next_run_time(self):
        with mock_datetime(2010, 1, 6, 12, 15):
            mock_job = make_mock_job()
            assert tz_schedule.next_run() is None
    ...
    def test_next_run_property(self):
        ...
        with mock_datetime(2010, 1, 6, 13, 16):
            ...
            assert len(tz_schedule.jobs) == 2
            # Make sure the hourly job is first
            assert tz_schedule.next_run() == original_datetime(2010, 1, 6, 14, 16)
            assert tz_schedule.idle_seconds() == 60 * 60
    ...
    def test_cancel_job(self):
        ...
        tz_schedule.run_all()
        assert len(tz_schedule.jobs) == 1
        assert tz_schedule.jobs[0] == mj

        tz_schedule.cancel_job('Not a job')
        assert len(tz_schedule.jobs) == 1
        tz_schedule.default_scheduler.cancel_job('Not a job')
        assert len(tz_schedule.jobs) == 1

        tz_schedule.cancel_job(mj)
        assert len(tz_schedule.jobs) == 0
    ...
    def test_clear_by_tag(self):
        ...
        tz_schedule.run_all()
        assert len(tz_schedule.jobs) == 3
        tz_schedule.clear('tag3')
        assert len(tz_schedule.jobs) == 2
        tz_schedule.clear('tag1')
        assert len(tz_schedule.jobs) == 0
    ...
    def test_misconfigured_job_wont_break_scheduler(self):
        ...
        """
        scheduler = tz_schedule.TimezoneAwareScheduler()
        scheduler.every()
